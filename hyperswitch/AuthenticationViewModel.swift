            ...
            do {
                ...
                guard let authId = json["authentication_id"] as? String else {
                    throw NSError(
                        domain: "API Error",
                        code: 500,
                        userInfo: [NSLocalizedDescriptionKey: "Missing authentication_id in response"]
                    )
                }
        ...
        Task { [weak self] in
            ...
            do {
                guard let authSessionId = self.authenticationSessionId else {
                    let error = NSError(
                        domain: "Missing Authentication Session",
                        code: 400,
                        userInfo: [NSLocalizedDescriptionKey: "Authentication session ID is required"]
                    )
                    DispatchQueue.main.async {
                        self.errorMessage = error.localizedDescription
                        completion(error)
            ...
            do {
                    ...
                    let threeDSRequestorAppURL = json["three_ds_requestor_url"] as? String?
                {
                    ...
                    throw NSError(
                        domain: "API Error",
                        code: 500,
                        userInfo: [NSLocalizedDescriptionKey: "Missing required challenge parameters in response"]
                    )
                }
            } catch {
                DispatchQueue.main.async {
