...
class PaymentMethodManagementViewController: UIViewController {
    ...
    private var tokenizeWidgetButton = UIButton()
    private var tokenizeWidgetButtonConfiguration = UIButton.Configuration.plain()
    ...
    private func presentSheet(_ sender: Any) {
        ...
        hyperViewModel.paymentMethodManagement?.present(
            from: self,
    ...
    @objc
    private func tokenizeWidget(_ sender: Any) {
        guard let pmmWidget = pmmWidget else { return }
        statusLabel.text = "tokenizing..."
        pmmWidget.tokenize { [weak self] result in
            DispatchQueue.main.async {
                self?.updateStatus(result)
            }
        }
    ...
    func attachPaymentMethodManagementWidget() {
        ...
        guard let paymentMethodManagement = hyperViewModel.paymentMethodManagement else { return }
        ...
        let widget = paymentMethodManagement.widget(configuration: configuration)
        ...
        widgetConstraints = [
            ...
            tokenizeWidgetButton.topAnchor.constraint(equalTo: widget.bottomAnchor, constant: 20),
        ]
...
extension PaymentMethodManagementViewController {
    ...
    func viewFrame() {
        ...
        tokenizeWidgetButton.setTitle("Save Card (tokenize)", for: .normal)
        tokenizeWidgetButton.setTitleColor(.white, for: .normal)
        tokenizeWidgetButtonConfiguration.contentInsets = NSDirectionalEdgeInsets(top: 10, leading: 10, bottom: 10, trailing: 10)
        tokenizeWidgetButton.configuration = tokenizeWidgetButtonConfiguration
        tokenizeWidgetButton.layer.cornerRadius = 10
        tokenizeWidgetButton.backgroundColor = .systemBlue
        tokenizeWidgetButton.addTarget(self, action: #selector(tokenizeWidget(_:)), for: .touchUpInside)
        contentView.addSubview(tokenizeWidgetButton)
        tokenizeWidgetButton.translatesAutoresizingMaskIntoConstraints = false
        tokenizeWidgetButton.leadingAnchor.constraint(equalTo: contentView.leadingAnchor, constant: 60).isActive = true
        tokenizeWidgetButton.trailingAnchor.constraint(equalTo: contentView.trailingAnchor, constant: -60).isActive = true
        tokenizeWidgetButton.bottomAnchor.constraint(equalTo: contentView.bottomAnchor, constant: -20).isActive = true
    }
}
